...
/*
  download & update XPs/votes (templeet version)

  nd poolpy: je précise que c'est (c) lordOric qui cuve,
  et que ton vi (kisux) a indenté le truc comme un puerco!
*/
static int
site_yc_parse_dacode2(Site *site, char *s)
{
  char *p;
  int err = 0;
  ...
  if (p != NULL) {
    ...
    if (regexp_extract( p, pat_xp_votes, &xp, &(site->votes_max), &(site->votes_cur))) {
    ...
    myprintf(_("[%<YEL %s>] Hmmm, how strange is '%s' ... there is no 'XPs:' field.\n"), 
...
*/
int
site_yc_parse_dacode14(Site *site, char *s)
{
  char *p, *p2, *p3;
  int err = 0;
...
site_yc_dl_and_update(Site *site) {
  ...
  if ((Prefs.debug & 2) == 0) {
    switch (site->type) {
    case DACODE14: snprintf(path, 2048, "%s%s/%s", (strlen(site->prefs->site_path) ? "/" : ""), 
			    site->prefs->site_path, site->prefs->path_myposts);
      break;
    case DACODE2: snprintf(path, 2048, "%s%s/my/", (strlen(site->prefs->site_path) ? "/" : ""), 
			   site->prefs->site_path );
      break;
    case SITE_UNKNOWN: myprintf("can't dl comments, site type unknown for the moment\n"); return;
    }
  } else {
  ...
  if (r.error == 0) {
    ...
    if (site->type == DACODE14)
      err = site_yc_parse_dacode14(site, s);
    else if (site->type == DACODE2)
      err = site_yc_parse_dacode2(site, s);
    free(s);
  ouups:
    if (err) {
