...
create_main_win (void)
{
  ...
  gtk_text_insert (GTK_TEXT (text1), NULL, NULL, NULL,
                   "WMC\262 2.4.5\n * commandes clavier dans le pinni (ouverture du palmi avec la touche enter etc)\n * un second bigorno d\351sactivable\n \n\nWMC\262 2.4.2\n* bugfixes divers -- tout fonctionne \340 nouveau sur dlfp, \340 l'exception du flamophone.\n\nWMC\262 2.4.1\n* ajout du menu contextuel\n* pour l'utilisation avec le templeet de linuxfr, voir http://hules.free.fr/wmcoincoin/rtff.html\n\nPASSAGE DE WMC\262 2.3.8 \340 WMC\262 2.4\n\nLe format du fichier d'options a beaucoup chang\351.. du coup il est conseill\351 d'utiliser le nouveau wmccc pour \351diter les options, c'est plus simple. Sinon, lisez le fichier d'options par d\351faut, c'est expliqu\351.\n\n* Pour ceux qui ne veulent pas se faire chier, il suffit de rajouter site: \"lenomdusite\" \340 la _FIN_ de fichier d'options que vous utilisez actuellement, et vous devriez retrouver un coincoin quasi similaire \340 la v2.3.8 (voir qd m\352me la remarque sur les cookies plus bas)\n\n* Les cookies:\nwmc\262 demande maintenant le NOM du cookie avec sa valeur, veillez donc \340 bien rajouter 'session_id=' devant la valeur du cookie.\n\n* Le bouton 'Apply' n'a pas forc\351ment un effet imm\351diat, si wmc\262 est occup\351 \340 d/l des news ou autre, il faudra attendre quelques secondes (le bouton wmc3 du pinnip\350de clignote)\n\n* Certaines rares options ne peuvent pas \352tre chang\351es en cours de fonctionnement (comme le skin de wmc\262)\n\n* IL EST TRES FORTEMENT CONSEILLE DE REPARTIR DU FICHIER D'OPTIONS PAR DEFAUT, ou alors d'ajouter \340 votre fichier d'options les diverses lignes 'url_replace' et 'board.miniua' du fichier d'options par d\351faut.\n\n* Les tabs du pinnipede ont trois \351tats:\n  off : on ne voit pas les messages du site\n  on : les messages du site sont affich\351s\n  on principale: c'est le site par d\351faut sur lequel vous postez avec le palmipede.\n\nLe comportement par d\351faut des tabs pour le clic gauche est le suivant\n  (1) tab off --> tab on principale\n  (2) tab on principale -> tous les autres \340 on\n  (3) tab on principale + tous les autres \340 on -> tous les autres \340 off\n  les clics suivants alternent entre (2) et (3)\n\nSi ce comportement vous d\351plait, cochez l'option 'classical tabs' dans le volet 'pinnip\350de' de wmccc.\n\nLes tabs du pinnipede ont aussi une petite croix en bas \340 droite: celle-ci permet d'annuler l'auto-refresh du site (plus de maj automatique de la tribune du site).", -1);
  ...
  gtk_box_pack_start (GTK_BOX (hbox168), checkbutton_board_auto_dl_pictures, FALSE, FALSE, 0);
  ...
  gtk_tooltips_set_tip (tooltips, text_post_cmd, "$l -> login, $m -> message, $u -> useragent, $i -> #id du message, $t -> timestamp, $s -> troll_score,  $R -> message type ( bit 0: message posted by you, bit 1: answer to one of your messages, bit 2: message selected by a keyword, bit 3: plopified/boitakoned message), $v -> wmc\262 version, $h -> remote host (REFER TO THE DEFAULT OPTIONS FILE FOR MORE DETAILS)", NULL);
